/*
	...
	関数一覧:
		...
		release_arasuzi: テスト関数
			...
			return [-1, null]
*/
import { sc_var, summary_sc_var, current_display, ASSETS, do_inaninstant, changeIsProcessingScenario, errorImageUrl, animManager, current_qr, scanned_qr, changeCurrentQr, camera_status, changeCssSvgUrl } from './engine.js';
let audioCtx = null;
...
const scenarioCommandActiveDisplays = ["story", "record_summary", "method", "ending"];
const SCENARIO_COMMAND_CANCELED = "scenario_command_canceled";
const isScenarioCommandDisplayActive = () => scenarioCommandActiveDisplays.includes(current_display);
...
const qr_logic = async () => {
	return new Promise((resolveLogic) => {
		...
		const cleanupAndResolve = async (result) => {
			...
			if (displayCancelTimer !== null) {
				clearInterval(displayCancelTimer);
				displayCancelTimer = null;
			}
		...
		displayCancelTimer = setInterval(() => {
			if (!isScenarioCommandDisplayActive()) cleanupAndResolve(SCENARIO_COMMAND_CANCELED);
		}, 250);
		...
		Html5Qrcode.getCameras()
			.then((cameras) => {
				if (isFinished || !isScenarioCommandDisplayActive()) return null;
...
export const COMMANDS = {
	...
	disp_text: (args) => { 
		...
		if (do_inaninstant) {
			const backlogName = has_chara_name ? args[4] : "";
			if (args[1] === "dialogue") {
				disp_area.textContent = args[2];
				return args[0] === "1" ? [31, [args[3], backlogName]] : [30, [args[3], backlogName]];
			}
			return args[0] === "1" ? [1, null] : [0, null];
		}
		const disp_content = replaceTemplateText(args[2]);
	...
	*/
	...
	change_classList: async (args) => {
		...
		if (!do_inaninstant) {
			//万が一のときのための防御
			changeIsProcessingScenario(true);
			await sleep(return_timer);
			changeIsProcessingScenario(false);
		}
		return args[0] === "1" ? [1, null] : [0, null];
	},
	...
	riddle: async (args) => {
		...
		const hasInvalidAnswers = setting.answers !== undefined && !Array.isArray(setting.answers);
		const correctAnswers = Array.isArray(setting.answers) ? setting.answers : (correctAnswer === undefined || correctAnswer === null ? [] : [correctAnswer]);
		const riddleStorageName = "riddle_results";
		const getRiddleKey = () => String(setting.key ?? setting.id ?? JSON.stringify([problemImagePath, successLine, correctAnswers.map((answer) => String(answer))]));
		...
		if (hasInvalidAnswers) {
			console.warn("riddle: answers はJSON配列で指定してください。");
			return finishReturn;
		}
		if (correctAnswers.length === 0) {
			console.warn("riddle: 正解文字列 answer または answers が指定されていません。");
			return finishReturn;
		}
		if (do_inaninstant) {
		...
		return new Promise((resolve) => {
			...
			(async () => {
				...
				finishRiddleOnError = finishRiddle;
				displayCancelTimer = setInterval(() => {
					if (!isScenarioCommandDisplayActive()) finishRiddle(finishReturn);
				}, 250);
				const askAnswer = () => {
					if (finished || !isScenarioCommandDisplayActive()) return;
					let message = String(setting.prompt ?? "答えを入力してください。");
					while (true) {
						const input = window.prompt(message);
						if (finished || !isScenarioCommandDisplayActive()) return;
						if (input === null) return;
						if (correctAnswers.some((answer) => String(input).trim() === String(answer).trim())) {
				...
				const openCameraDisplay = async () => {
					...
					try {
						...
						if (finished || !isScenarioCommandDisplayActive()) {
				...
				if (typeof finishRiddleOnError === "function") finishRiddleOnError(finishReturn);
				else resolve(finishReturn);
			});
		});
	},
	qr: async (args) => {
		...
		return new Promise((resolve) => {
			...
			displayCancelTimer = setInterval(() => {
				if (!isScenarioCommandDisplayActive()) finishQrCommand([0, null]);
			}, 250);
			...
			const finishQrRead = (qrRead) => {
				if (finished) return;
				if (qrRead === SCENARIO_COMMAND_CANCELED || !isScenarioCommandDisplayActive()) {
					finishQrCommand([0, null]);
					return;
				}
	...
	rends: async (args) => {
		if (do_inaninstant) return args[0] === "1" ? [1, null] : [0, null];
		const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
