/*
	...
	関数一覧:
		...
		release_arasuzi: テスト関数
			...
			return [-1, null]
*/
import { sc_var, summary_sc_var, current_display, ASSETS, do_inaninstant, current_qr, scanned_qr, changeCurrentQr, camera_status } from './engine.js';
let audioCtx = null;
...
const qr_logic = async () => {
    return new Promise((resolveLogic) => {
        ...
        const cleanupAndResolve = async (result) => {
            ...
            // カメラ停止・クリア処理（スコープ内の変数を利用するため、必要に応じて外側から参照するかここで完結させる）
            if (window._currentHtml5QrCode && window._isCameraActive) {
                try {
                    await window._currentHtml5QrCode.stop();
                    await window._currentHtml5QrCode.clear();
                } catch (e) {
                    console.error("カメラ停止エラー:", e);
                }
                window._isCameraActive = false;
            }
            resolveLogic(result);
        };
        ...
        if (backButton) {
            backButton.addEventListener("click", () => {
                cleanupAndResolve("back_button_pusshed");
            });
        }

        // html5-qrcodeのインスタンス化 (div#video_div を利用)
        window._currentHtml5QrCode = new Html5Qrcode("video_div");
        ...
        // カメラ起動開始
        window._currentHtml5QrCode.start({ facingMode: "environment" }, config, qrCodeSuccessCallback)
            .then(() => {
                window._isCameraActive = true;
            })
            .catch((err) => {
...
export const COMMANDS = {
	...
	*/
	...
	qr: async (args) => {
		...
		//続きから再生時の処理
		if (do_inaninstant) {
			console.log("current_qr", current_qr, "scanned_qr", scanned_qr);
			let next_qr = null;
			if (current_qr === "before_begin" && scanned_qr.includes("2")) next_qr = "2";//後で合わせて修正
			else {
				const last_qr_index = scanned_qr.indexOf(current_qr);
				if (last_qr_index === -1) {
					console.warn("不正です。qr", current_qr, "は実行実績がありません");
					return [-1, null];
				} else if (scanned_qr[last_qr_index+1] === undefined) {
					console.warn("qr",current_qr , "の次が既に実行済みであるはずですが実行実績がありません");
					return [-1, null];
				}
				next_qr = scanned_qr[last_qr_index+1];
			}
			if (next_qr === null) {
				console.warn("実行実績のないqrです");
				return [-1, null];
			}
			return [2, settingsJson.qr_info_table[next_qr].start];
		}
		return new Promise((resolve) => {
			const qrdiv = document.getElementById("between_frame_and_menu");
			const parser = new DOMParser();
			const routehtml = parser.parseFromString(ASSETS.html["HTML/maindisplay/route.html"], "text/html").body.innerHTML;
			qrdiv.innerHTML = routehtml;
			...
			let qr_read = null;
			document.getElementById("route_qr").addEventListener('click', async () => {
				cameradiv.classList.add('is_open');
				//実際の処理
				if (camera_status !== "granted") {
					console.log("カメラが非許可");
					qr_read = window.prompt("コードを入力");
				} else {
					qr_read = await qr_logic();
				}
				//判定
				console.log("scanned_qr =", qr_read);
				if (qr_read === "back_button_pusshed" || qr_read === null) {
					cameradiv.classList.remove('is_open');
				} else {
					qrdiv.classList.remove('is_open');
					qrdiv.innerHTML = "";
					scanned_qr.push(args[0]);
					changeCurrentQr(args[0]);
					localStorage.setItem("scanned_qr", JSON.stringify(scanned_qr));
					document.getElementById("front_item_canvas").style.display = "display";
					cameradiv.classList.remove('is_open');
					cameradiv.innerHTML = "";
					resolve([2, settingsJson.qr_info_table[args[0]].start]);//簡易版
				}
			});
			const mapurl = settingsJson.qr_info_table[args[0]].map;
			document.getElementById("route_content").style.backgroundImage = `url(${ASSETS.imgs[mapurl]})`;
			qrdiv.classList.add('is_open');
			document.getElementById("front_item_canvas").style.display = "none";
