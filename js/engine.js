...
function preloadHtml5Qrcode() {
  if (isHtml5QrcodeLoaded) return Promise.resolve();
  
  return new Promise((resolve, reject) => {
    const script = document.createElement('script');
    script.src = './js/html5-qrcode.min.js'; // ローカル（オフライン対応）のパス
    script.onload = () => {
      isHtml5QrcodeLoaded = true;
      resolve();
    };
...
async function display_change_toStory() {
		...
		document.getElementById("mainmenubutton").addEventListener('click', async () => {
			...
			const skip_logic = async () => {
				if (current_display === "record_summary") return; //万が一の防衛
			...
			if (current_display === "record_summary") skipButton.disabled = true;
