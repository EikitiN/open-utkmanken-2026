		...
		.then(text => {
			...
			if (last_saved_scenario >= 0) {
				for (let i = 0; i < SCENARIO.length; i++) {
					if (SCENARIO[i][0] === "release_arasuzi") {
						const arasuzi_num = Number(SCENARIO[i][1][1]);
						if (!Number.isInteger(arasuzi_num) || arasuzi_num < 1) {
							console.warn(`release_arasuzi: 保存済み進行から復元するあらすじ番号 '${SCENARIO[i][1][1]}' が不正です。`);
						} else {
...
async function first_display_change() {
	...
	const record_summary_disp = async (arasuzi_num) => {
		...
		document.getElementById("replay_story").addEventListener('click', async () => {
			...
			last_sc_line_inSummary = do_scenario(record_summary_do_scenario[arasuzi_num][0]-1);
		});
	}
...
async function display_change_toStory() {
		if (current_display !== "record_summary") current_display = "story";
		...
		document.getElementById("mainmenubutton").addEventListener('click', async () => {
			...
			const skip_logic = async () => {
				...
				const current_sc = current_display !== "record_summary" ? last_sc_line : last_sc_line_inSummary;
				const ExistScNum = SCENARIO.length - 1;
				let next_stop = ExistScNum;//この0-basedに次の表示すべきscが入る
				let index_s = 0;
				const stopping_funcName = ["end", "qr", "puzzle"];
				while (current_sc + index_s <= ExistScNum) {
					if (stopping_funcName.includes(SCENARIO[current_sc + index_s][0])) {
			...
			document.getElementById("backtotitle").addEventListener('click', () => {
				if (current_display === "record_summary") gotoRecord(); 
				else gotoTitle();
			});
		});
		//バックログ(探偵ログ)
		document.getElementById("backlogbutton").addEventListener('click', async () => {
			...
			if (backlog.length === 0) {//中身が何もなかったら
				...
				for (const OneLog of backlog) {
					...
					const charaimgurl = ASSETS.imgs[`material/mainmenu_icon/chara_icon_${OneLog[1][0].substring(0, OneLog[1][0].indexOf('_'))}.jpg`] ?? errorImageUrl;
		...
		//オートモードの機能
		//タイプライタ―演出が終わり、次がdo_nextでもなくなるまで待つ
		const waitForFalse = async (checkInterval = 50) => {
			if (!typewriter_flag && !isDoNext) return;
			return new Promise((resolve) => {
				const timer = setInterval(() => {
				if (!typewriter_flag && !isDoNext) {
					clearInterval(timer);
					resolve();
				}
				}, checkInterval);
		...
		const automode_stop_command = ["end", "qr", "puzzle"];
		async function automode() {
			const index_in_automode = current_display !== "record_summary" ? last_sc_line + 1 : last_sc_line_inSummary + 1;
			const [functionname, args] = SCENARIO[index_in_automode] ?? ["end", []];
		...
		//最初のシナリオを表示する
		await sleep(10);
		if (!(Number(localStorage.getItem("last_sc_line") || -1) >= 0) && (current_display !== "record_summary")) {
			//do_inaninstant = true;
...
async function do_scenario(num) {
	...
	function adjustTextDisplay() {
		...
		const text_typewriter = async () => {
			...
			for (let char_num = 1; char_num < charIndex; char_num++) {
				const char_elm = document.getElementById(`char-${char_num}`);
	...
	if (WfTDA_global) {
		...
		while (true) {
			...
			if (funcName === "qr" || funcName === "puzzle") isQrOrPuzzle = true;
			else isQrOrPuzzle = false;

			if (typeof commandFunc === "function") {
				const [code_original, value] = commandFunc(args);
				...
				// setvar 系の保存
				if (code === 10 || code === 11) {
					if (current_display !== "record_summary") saveScVarToDB();
				}
				// ジャンプ
				if (code === 2) {
					index = value; // 0-based
				...
				//テキスト(セリフ)
				if ((code === 30 || code === 31) && current_display !== "record_summary") backlog.push([backlog.length, value, document.getElementById("textcontent").textContent]);//[番号, [キャラid, キャラ名], 本文]
