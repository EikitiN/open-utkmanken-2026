...
let isProcessingScenario = false;
let isBlockNextButtoAsProcDoScenario = false;
...
export const changeIsProcessingScenario = (flag) => {
	//isOnScenario = flag;
	isProcessingScenario = flag;
}
//次へボタンの管理
let nextButtonIntervalId = null;
let isBlockingNextButton = false;
const NextButtonManager = () => {
	if (!["story", "record_summary", "method", "ending"].includes(current_display)) return;
	const nextbutton = document.getElementById("nextbutton");
	if (!nextbutton) {
		console.log("nextbuttonはありません");
		return;
	}
	if (nextButtonIntervalId) {
        clearInterval(nextButtonIntervalId);
        nextButtonIntervalId = null;
    }
	const checkFlags = setInterval(() => {
		if (!["story", "record_summary", "method", "ending"].includes(current_display) || !nextbutton) {
			console.log("次へボタン監視終了");
			clearInterval(checkFlags);
			return;
		}
		if (nextbutton.disabled) {
			if (!(isAnimating || typewriter_flag || isDoNext || isQrOrPuzzle || isProcessingScenario || isBlockNextButtoAsProcDoScenario)) {//監視対象
				nextbutton.disabled = false;
				isBlockingNextButton = false;
			}
		}
	}, 50);
}
...
const loadOneFile = async (db, storeName, path, version, updateProgress) => {
	...
	if (!res.ok) {
		console.error("正しくロードされませんでした:", path, "HTTPステータスコード:", res.status);
		return;
	}
...
const loadOneScenario = async (db, path, version) => {
	...
	const text = await fetch(path, { cache: 'no-store' })
    .then(r => {
        if (!r.ok) {
			console.error("シナリオファイルの読み込み不全:", path, "HTTPステータスコード:", r.status);
            throw new Error(`Failed to fetch scenario: ${path} (status: ${r.status})`);
        }
        return r.text();
    });
...
async function first_display_change() {
	...
	const ending_story = async () => {
		...
		//ゲーム画面の用意
		current_display = "ending";
		summary_sc_var = {};
		isOnScenario = true;
		await display_change_toStory();
		//シーン切り替え
		do_inaninstant = true;
		for (let i = 0; i < record_summary_do_scenario[-1][0]-1; i++) {
			let returned_index = await do_scenario(i);
			i = returned_index;
			last_sc_line_inSummary = i;
		}
		do_inaninstant = false;
		last_sc_line_inSummary = await do_scenario(record_summary_do_scenario[-1][0]-1);
	}
...
async function display_change_toStory() {
		if (current_display !== "record_summary" && current_display !== "method" && current_display !== "ending") current_display = "story";
		...
		document.getElementById("mainmenubutton").addEventListener('click', async () => {
			...
			const skip_logic = async () => {
				if (current_display === "record_summary" || current_display === "method" || current_display === "ending") return; //万が一の防衛
				if (!window.confirm("ストーリーをスキップしますか")) return;
				skipButton.disabled = true;
				const current_sc = current_display !== "record_summary" && current_display !== "method" && current_display !== "ending" ? last_sc_line : last_sc_line_inSummary;
				const ExistScNum = SCENARIO.length - 1;
				let next_stop = ExistScNum;//この0-basedに次の表示すべきscが入る
				let index_s = 0;
				const stopping_funcName = ["end", "qr", "puzzle", "riddle", "ending"];
				...
				for (let i = current_sc + 1; i < next_stop; i++) {
					let returned_index = await do_scenario(i);
					i = returned_index;
					if (current_display !== "record_summary" && current_display !== "method" && current_display !== "ending") {
						last_sc_line = i;
						localStorage.setItem("last_sc_line", String(last_sc_line));
					} else {
						last_sc_line_inSummary = i;
			...
			if (current_display === "record_summary" || current_display === "method" || current_display === "ending") skipButton.disabled = true;
			skipButton.addEventListener('click', skip_logic);
			document.getElementById("backtotitle").addEventListener('click', () => {
				if (current_display === "record_summary" || current_display === "method" || current_display === "ending") gotoRecord(); 
				else gotoTitle();
			});
		});
		//バックログ(探偵ログ)
		...
		const next_button_func = async (e) => {
			...
			if (isBlockNextButtoAsProcDoScenario) {
				console.warn("next_button_func 重複実行拒否");
				return;
			} //重複実行禁止
			isBlockNextButtoAsProcDoScenario = true;
			//console.log("nextbutton current_display: ", current_display);
			document.getElementById("nextbutton").disabled = true;
			isBlockingNextButton = true;
			if (current_display !== "record_summary" && current_display !== "method" && current_display !== "ending") {
			...
			isBlockNextButtoAsProcDoScenario = false;
			/*
		...
		*/
		const waitForFalse = async (checkInterval = 50) => {
			if (!isBlockingNextButton) return;
			return new Promise((resolve) => {
				const timer = setInterval(() => {
					if (!isBlockingNextButton) {
						clearInterval(timer);
						resolve();
					}
				}, checkInterval);
		...
		const automode_stop_command = ["end", "qr", "puzzle", "riddle", "ending"];
		async function automode() {
			const index_in_automode = current_display !== "record_summary" && current_display !== "method" && current_display !== "ending" ? last_sc_line + 1 : last_sc_line_inSummary + 1;
		...
		await sleep(10);
		NextButtonManager();
		if (!(Number(localStorage.getItem("last_sc_line") || -1) >= 0) && (current_display !== "record_summary" && current_display !== "method" && current_display !== "ending")) {
			//do_inaninstant = true;
...
async function do_scenario(num) {
	...
	function adjustTextDisplay() {
		...
		if (mainmenubutton) mainmenubutton.disabled = false;
		if (hiddenText !== "") {
	...
	const waitForFalse = async (checkInterval = 50) => {
		if ((!typewriter_flag && isOnScenario) || !["story", "record_summary", "method", "ending"].includes(current_display)) return;
		return new Promise((resolve) => {
			const timer = setInterval(() => {
			if ((!typewriter_flag && isOnScenario) || !["story", "record_summary", "method", "ending"].includes(current_display)) {
				clearInterval(timer);
				resolve();
			}
	...
	try {
		while (true) {
			...
			if (typeof commandFunc === "function") {
				//console.log("コマンド実行:", funcName, args);
				const [code_original, value] = await commandFunc(args);
				let code = code_original;
				//console.log("コマンドから返り値が返ってきた:", funcName, code_original, value);
				isQrOrPuzzle = false;
				...
				// setvar 系の保存
				if (code === 10 || code === 11) {
					if (current_display !== "record_summary" && current_display !== "method" && current_display !== "ending") await saveScVarToDB();
				}
				// ジャンプ
				if (code === 2) {
					index = value; // 0-based
				...
				// 次へ
				if (code === 1 || code === 11) {
					if (current_display !== "story" && current_display !== "record_summary" && current_display !== "method" && current_display !== "ending") {
				...
				//テキスト(セリフ)
				if ((code === 30 || code === 31) && current_display !== "record_summary" && current_display !== "method" && current_display !== "ending") backlog.push([backlog.length, value, document.getElementById("textcontent").textContent]);//[番号, [キャラid, キャラ名], 本文]
